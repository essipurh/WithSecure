...
def batches_generator(records: List[str]) -> Generator:
  ...
  for record in records:
    record_size = len(record.encode('utf-8')) # the records are assumed to be strings, could use sys.getsizeof if non string
    if  record_size > MAX_RECORD_SIZE:
      continue

    if (record_size + current_batch_size > MAX_BATCH_SIZE) or (len(current_batch) >= MAX_RECORDS_PER_BATCH):
      yield current_batch
      current_batch = []
      current_batch_size = 0
